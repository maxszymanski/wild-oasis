...
function SalesChart({ bookings, numDays }) {
    ...
    // zwraca tablicę dat w określonym przedziale czasu
    const allDates = eachDayOfInterval({
        start: subDays(new Date(), numDays - 1), // obliczamy dziś minus ilosć wybranych dni
        end: new Date(),
    })

    const data = allDates.map((date) => {
        return {
            label: format(date, 'MMM dd'), // ustalamy format na miesiąc, dzien
            totalSales: bookings
                .filter((booking) =>
                    isSameDay(date, new Date(booking.created_at))
                )
                .reduce((acc, cur) => acc + cur.totalPrice, 0), // zwracamy tylko te bookingi w dni które są w przedziale wybranych dni i sumujemy ich totalPrice
            extrasSales: bookings
                .filter((booking) =>
                    isSameDay(date, new Date(booking.created_at))
                )
                .reduce((acc, cur) => acc + cur.extrasPrice, 0), // zwracamy tylko te bookingi w dni które są w przedziale wybranych dni i sumujemy ich extrasSales
        }
    })
